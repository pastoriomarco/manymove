...
- Mounts your host workspace `src/` directory into the container at `/opt/manymove_ws/src` by default, so edits in your local checkout are visible immediately inside Docker without replacing the image's prebuilt `install/` overlay. Set `MANYMOVE_ROS_WS` if auto-detection should use a different workspace root.
...
Adjust build arguments (e.g., `MANYMOVE_BRANCH`) and set `ROS_DISTRO=jazzy` for the Jazzy image. The image still builds its own fallback workspace at `/opt/manymove_ws`, and the helper scripts now bind-mount your host `src/` tree into that workspace by default when they can resolve it.
