...
int main(int argc, char **argv)
{
    ...
    * robot to plan while checking for collisions
    ...
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    // std::vector<double> load_post_transform_xyz_rpy_2 = {0.0, 0.0, 0.1-tube_length/3, 0.0, 0.0, 0.0}; // to check for attached object collision
    // std::vector<double> load_post_transform_xyz_rpy_2 = {0.0, 0.0, 0.2-tube_length/3, 0.0, 0.0, 0.0}; // to check for gripper collision
    std::vector<double> load_post_transform_xyz_rpy_2 = {0.1, 0.1, 0.2-tube_length/3, 0.0, 0.0, 0.0}; // to check for BIG collision
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    ...
    std::string wait_move_parallel_1_xml = buildMoveXML(
        rp_1.prefix, rp_1.prefix + "wait", wait_position_1, blackboard);

    std::string exit_move_parallel_1_xml = buildMoveXML(
        rp_1.prefix, rp_1.prefix + "exit", exit_position_1, blackboard);
    ...
    std::string insert_move_parallel_2_xml = buildMoveXML(
        rp_2.prefix, rp_2.prefix + "pick", insert_sequence_2, blackboard);

    std::string load_move_parallel_2_xml = buildMoveXML(
        rp_2.prefix, rp_2.prefix + "drop", load_sequence_2, blackboard);

    std::string exit_move_parallel_2_xml = buildMoveXML(
        rp_2.prefix, rp_2.prefix + "exit", exit_sequence_2, blackboard);

    std::string ready_move_parallel_2_xml = buildMoveXML(
        rp_2.prefix, rp_2.prefix + "toReady", ready_position_2, blackboard);
