...
public:
        ...
        : Node("collision_spawner"), rng_(std::random_device{}())
    {
        ...
        {
            RCLCPP_WARN(this->get_logger(), "CheckObjectExists action server not available. Proceeding without existence checks.");
        }
        else
        {
...
private:
    ...
    void sendRemoveGoal(const std::string &object_id)
    {
        ...
        if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), send_goal_future, std::chrono::seconds(5)) !=
        ...
        if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), get_result_future, std::chrono::seconds(5)) !=
            rclcpp::FutureReturnCode::SUCCESS)
        {
    ...
    bool isObjectPresent(const std::string &object_id)
    {
        ...
        // Wait for the goal to be accepted
        if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), send_goal_future, std::chrono::seconds(3)) !=
            rclcpp::FutureReturnCode::SUCCESS)
        {
            RCLCPP_WARN(this->get_logger(),
        ...
        if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), get_result_future, std::chrono::seconds(3)) !=
            rclcpp::FutureReturnCode::SUCCESS)
        {
            RCLCPP_WARN(this->get_logger(),
