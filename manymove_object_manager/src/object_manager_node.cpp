...
: Node("object_manager_node")
{
  ...
  add_object_action_server_ = rclcpp_action::create_server<AddCollisionObject>(
    ...
    std::bind(
      &ObjectManagerNode::handleAddGoal, this, std::placeholders::_1,
      std::placeholders::_2),
  ...
  remove_object_action_server_ = rclcpp_action::create_server<RemoveCollisionObject>(
    ...
    std::bind(
      &ObjectManagerNode::handleRemoveGoal, this, std::placeholders::_1,
      std::placeholders::_2),
  ...
  check_object_exists_action_server_ = rclcpp_action::create_server<CheckObjectExists>(
    ...
    std::bind(
      &ObjectManagerNode::handleCheckExistGoal, this, std::placeholders::_1,
      std::placeholders::_2),
    std::bind(&ObjectManagerNode::handleCheckExistCancel, this, std::placeholders::_1),
    std::bind(
      &ObjectManagerNode::handleCheckExistExecute, this,
      std::placeholders::_1), serverOptions,
    action_callback_group_);
  ...
  attach_detach_object_action_server_ = rclcpp_action::create_server<AttachDetachObject>(
    ...
    std::bind(
      &ObjectManagerNode::handleAttachDetachExecute, this,
      std::placeholders::_1), serverOptions,
    action_callback_group_);
  ...
  get_object_pose_action_server_ = rclcpp_action::create_server<GetObjectPose>(
    ...
    std::bind(
      &ObjectManagerNode::handleGetObjectPoseExecute, this,
      std::placeholders::_1), serverOptions,
    action_callback_group_);
  ...
  if (future_response.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready) {
    ...
    if (response) {
      ...
      if (collision_objects.empty()) {
        ...
        RCLCPP_INFO(
          this->get_logger(), "  Found %zu collision object(s):",
...
rclcpp_action::CancelResponse
ObjectManagerNode::handleAddCancel(
...
void ObjectManagerNode::handleAddExecute(const std::shared_ptr<AddGoalHandle> goal_handle)
{
  ...
  auto collision_object =
    ...
    goal->id, goal->shape, goal->dimensions, goal->pose, goal->mesh_file,
    goal->scale_mesh);
  ...
  const std::shared_ptr<AttachDetachGoalHandle> goal_handle)
{
  ...
  if ((!attached_object_exists && !collision_object_exists)) {
    result->success = false;
    result->message = std::string("'") + goal->object_id +
  ...
  if (goal->attach) {
    ...
    if (!collision_object_opt) {
      RCLCPP_ERROR(
        this->get_logger(), "Failed to retrieve object '%s' data.",
        goal->object_id.c_str());
  ...
  const std::shared_ptr<GetObjectPoseGoalHandle> goal_handle)
{
  ...
  try {
    ...
    tf2::Quaternion obj_quat(object_pose.orientation.x, object_pose.orientation.y,
...
bool ObjectManagerNode::attachedObjectExists(const std::string & id, const std::string & link_name)
{
  ...
  if (link_name.size() > 0) {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Checking if object '%s' is attached to any link...",
      id.c_str());
  }
  ...
  const std::string & object_id)
{
  ...
  if (future_response.wait_for(std::chrono::seconds(5)) == std::future_status::ready) {
    ...
    if (response) {
      ...
      RCLCPP_WARN(
        this->get_logger(), "Object '%s' not found in planning scene.",
        object_id.c_str());
    } else {
  ...
  if (shape == "mesh") {
    ...
    try {
      ...
      if (auto mesh_ptr = boost::get<shape_msgs::msg::Mesh>(&shape_msg)) {
        ...
        RCLCPP_ERROR(
          this->get_logger(), "Failed to extract mesh from ShapeMsg for object '%s'.",
          id.c_str());
      }
    } catch (const std::exception & e) {
    ...
    if (shape == "box") {
      ...
      RCLCPP_WARN(
        this->get_logger(), "Unsupported shape type: %s. Defaulting to box.",
        shape.c_str());
      primitive.type = primitive.BOX;
      primitive.dimensions.resize(3);
      primitive.dimensions[0] = 0.1;
