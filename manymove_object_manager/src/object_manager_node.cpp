...
namespace manymove_object_manager
{
  ...
  ObjectManagerNode::ObjectManagerNode() : Node("object_manager_node")
  {
    ...
    attached_collision_object_publisher_ =
        this->create_publisher<moveit_msgs::msg::AttachedCollisionObject>("/attached_collision_object", 10);
    ...
    add_object_action_server_ = rclcpp_action::create_server<AddCollisionObject>(
        ...
        std::bind(&ObjectManagerNode::handleAddExecute, this, std::placeholders::_1), serverOptions,
    ...
    remove_object_action_server_ = rclcpp_action::create_server<RemoveCollisionObject>(
        ...
        std::bind(&ObjectManagerNode::handleRemoveExecute, this, std::placeholders::_1), serverOptions,
        action_callback_group_);
    ...
    check_object_exists_action_server_ = rclcpp_action::create_server<CheckObjectExists>(
        ...
        std::bind(&ObjectManagerNode::handleCheckExistExecute, this, std::placeholders::_1), serverOptions,
        action_callback_group_);

    attach_detach_object_action_server_ = rclcpp_action::create_server<AttachDetachObject>(
        this, "attach_detach_object",
        ...
        std::bind(&ObjectManagerNode::handleAttachDetachExecute, this, std::placeholders::_1), serverOptions,
        action_callback_group_);

    get_object_pose_action_server_ = rclcpp_action::create_server<GetObjectPose>(
        this, "get_object_pose",
        ...
        std::bind(&ObjectManagerNode::handleGetObjectPoseExecute, this, std::placeholders::_1), serverOptions,
        action_callback_group_);
  ...
  ObjectManagerNode::handleCheckExistGoal([[maybe_unused]] const rclcpp_action::GoalUUID &uuid,
                                          std::shared_ptr<const CheckObjectExists::Goal> goal)
  {
    RCLCPP_INFO(this->get_logger(), "Received request to check existence of object: %s", goal->object_id.c_str());
  ...
  void ObjectManagerNode::handleCheckExistExecute(const std::shared_ptr<CheckGoalHandle> goal_handle)
  {
    ...
    if (attached_object_exists)
    {
      ...
      RCLCPP_INFO(this->get_logger(), "CheckExist: Object '%s' exists attached to link '%s'.", goal->object_id.c_str(),
                  result->link_name.c_str());
    }
    else
    {
      result->link_name = "";
      result->message = exists ? "Object exists in the planning scene." : "Object not found in the planning scene.";

      RCLCPP_INFO(this->get_logger(), "CheckExist: Object '%s' %s in the planning scene.", goal->object_id.c_str(),
      ...
      if (response)
      {
        ...
        else
        {
          RCLCPP_INFO(this->get_logger(),
                      "  Found %zu attached collision object(s):", attached_collision_object.size());
  ...
  // ----------------------------------------------------------------------------
  rclcpp_action::GoalResponse ObjectManagerNode::handleAddGoal([[maybe_unused]] const rclcpp_action::GoalUUID &uuid,
  ...
  rclcpp_action::CancelResponse
  ObjectManagerNode::handleAddCancel([[maybe_unused]] const std::shared_ptr<AddGoalHandle> goal_handle)
  {
    RCLCPP_INFO(this->get_logger(), "Add operation canceled.");
    return rclcpp_action::CancelResponse::ACCEPT;
  }

  void ObjectManagerNode::handleAddExecute(const std::shared_ptr<AddGoalHandle> goal_handle)
  {
    auto goal = goal_handle->get_goal();
    ...
    auto collision_object =
        createCollisionObject(goal->id, goal->shape, goal->dimensions, goal->pose, goal->mesh_file, goal->scale_mesh);
    ...
    for (int attempt = 1; attempt <= max_retries; ++attempt)
    {
      ...
      if (exists)
      {
        ...
        goal_handle->succeed(result);
        RCLCPP_INFO(this->get_logger(), "Object '%s' added successfully after %d attempt(s).", goal->id.c_str(),
                    attempt);
        return;
      }

      RCLCPP_WARN(this->get_logger(), "Attempt %d: Object '%s' not found in planning scene yet.", attempt,
                  goal->id.c_str());
    }
    ...
    RCLCPP_ERROR(this->get_logger(), "Failed to verify addition of object '%s' after %d attempts.", goal->id.c_str(),
                 max_retries);
  }
  ...
  // ----------------------------------------------------------------------------
  rclcpp_action::GoalResponse
  ObjectManagerNode::handleRemoveGoal([[maybe_unused]] const rclcpp_action::GoalUUID &uuid,
  ...
  void ObjectManagerNode::handleRemoveExecute(const std::shared_ptr<RemoveGoalHandle> goal_handle)
  {
    ...
    collision_object_publisher_->publish(collision_object);
    RCLCPP_INFO(this->get_logger(), "Published REMOVE operation for object '%s'.", goal->id.c_str());
    ...
    for (int attempt = 1; attempt <= max_retries; ++attempt)
    {
      ...
      if (!exists)
      {
        ...
        goal_handle->succeed(result);
        RCLCPP_INFO(this->get_logger(), "Object '%s' removed successfully after %d attempt(s).", goal->id.c_str(),
                    attempt);
        return;
      }

      RCLCPP_WARN(this->get_logger(), "Attempt %d: Object '%s' still exists in planning scene.", attempt,
                  goal->id.c_str());
    ...
    goal_handle->abort(result);
    RCLCPP_ERROR(this->get_logger(), "Failed to verify removal of object '%s' after %d attempts.", goal->id.c_str(),
                 max_retries);
  }
  ...
  // ----------------------------------------------------------------------------
  rclcpp_action::GoalResponse
  ObjectManagerNode::handleAttachDetachGoal([[maybe_unused]] const rclcpp_action::GoalUUID &uuid,
                                            std::shared_ptr<const AttachDetachObject::Goal> goal)
  {

    RCLCPP_INFO(this->get_logger(), "Received request to %s object: %s to link: %s", goal->attach ? "attach" : "detach",
                goal->object_id.c_str(), goal->link_name.c_str());

    // Basic validation
    if (goal->object_id.empty() || (goal->attach && goal->link_name.empty()))
    {
      RCLCPP_WARN(this->get_logger(), "Invalid object_id or link_name in the request.");
      return rclcpp_action::GoalResponse::REJECT;
    }
  ...
  void ObjectManagerNode::handleAttachDetachExecute(const std::shared_ptr<AttachDetachGoalHandle> goal_handle)
  {
    auto goal = goal_handle->get_goal();
    ...
    if (goal->attach && (attached_object_exists))
    {
      ...
      {
        result->success = true;
        result->message = std::string("'") + goal->object_id + "' already attached to " +
    ...
    else
    {
      ...
      {
        RCLCPP_ERROR(this->get_logger(), "Failed to retrieve object '%s' data.", goal->object_id.c_str());

        result->success = false;
        result->message = goal->attach ? "Failed to attach object." : "Failed to detach object.";
        goal_handle->abort(result);
        RCLCPP_ERROR(this->get_logger(), "%s operation failed for object '%s' and link '%s'.",
                     goal->attach ? "Attach" : "Detach", goal->object_id.c_str(), goal->link_name.c_str());
        return;
      }
    }
    ...
    for (int attempt = 1; attempt <= max_retries; ++attempt)
    {
      ...
      if (future_response.wait_for(std::chrono::milliseconds(500)) == std::future_status::ready)
      {
        ...
        if (response)
        {
          ...
          {
            RCLCPP_INFO(this->get_logger(), "Object '%s' successfully attached to link '%s'.", goal->object_id.c_str(),
          ...
          else if (!goal->attach && !found)
          {
            ...
            if (!objectExists(goal->object_id))
            {
              ...
              collision_object_publisher_->publish(collision_object);
              RCLCPP_INFO(this->get_logger(), "Re-added object '%s' to the planning scene.", goal->object_id.c_str());
              ...
              if (objectExists(goal->object_id))
              {
                RCLCPP_INFO(this->get_logger(), "Object '%s' re-added to the planning scene successfully.",
                            goal->object_id.c_str());
                operation_successful = true;
                break;
              }
              ...
              {
                RCLCPP_WARN(this->get_logger(), "Failed to re-add object '%s' to the planning scene.",
                            goal->object_id.c_str());
              }
            }
            else
      ...
      RCLCPP_WARN(this->get_logger(), "Attempt %d: Operation not confirmed yet.", attempt);
    }
    ...
    if (operation_successful)
    {
      ...
      goal_handle->succeed(result);
      RCLCPP_INFO(this->get_logger(), "%s operation completed successfully for object '%s' and link '%s'.",
                  goal->attach ? "Attach" : "Detach", goal->object_id.c_str(), goal->link_name.c_str());
    }
    else
    {
  ...
  rclcpp_action::GoalResponse
  ObjectManagerNode::handleGetObjectPoseGoal([[maybe_unused]] const rclcpp_action::GoalUUID &uuid,
                                             std::shared_ptr<const GetObjectPose::Goal> goal)
  {
    RCLCPP_INFO(this->get_logger(), "Received GetObjectPose goal for object: %s, reference link: %s",
                goal->object_id.c_str(), goal->link_name.c_str());
  ...
  void ObjectManagerNode::handleGetObjectPoseExecute(const std::shared_ptr<GetObjectPoseGoalHandle> goal_handle)
  {
    auto goal = goal_handle->get_goal();
    ...
    if (is_attached)
    {
      ...
      if (!maybe_link)
      {
        RCLCPP_ERROR(this->get_logger(), "Failed to get attached link for object '%s'.", goal->object_id.c_str());
        result->success = false;
    ...
    if (!goal->link_name.empty() && goal->link_name != object_frame)
    {
      ...
      {
        RCLCPP_ERROR(this->get_logger(), "Failed to lookup transform from '%s' to '%s': %s", object_frame.c_str(),
    ...
    try
    {
      ...
      pre_rot.setRPY(goal->pre_transform_xyz_rpy[3], goal->pre_transform_xyz_rpy[4], goal->pre_transform_xyz_rpy[5]);
      ...
      tf2::Vector3 pre_trans(goal->pre_transform_xyz_rpy[0], goal->pre_transform_xyz_rpy[1],
      ...
      tf2::Vector3 post_trans(goal->post_transform_xyz_rpy[0], goal->post_transform_xyz_rpy[1],
      ...
      tf2::Quaternion obj_quat(object_pose.orientation.x, object_pose.orientation.y, object_pose.orientation.z,
      ...
      tf2::Vector3 obj_trans(object_pose.position.x, object_pose.position.y, object_pose.position.z);
      ...
      RCLCPP_INFO(this->get_logger(),
                  ...
                  goal->object_id.c_str(), final_pose.position.x, final_pose.position.y, final_pose.position.z,
  ...
  bool ObjectManagerNode::attachedObjectExists(const std::string &id, const std::string &link_name)
  {
    ...
    if (link_name.size() > 0)
    {
      RCLCPP_INFO(this->get_logger(), "Checking if object '%s' is attached to link '%s'...", id.c_str(),
                  link_name.c_str());
    }
    else
    {
    ...
    if (future_response.wait_for(std::chrono::seconds(5)) == std::future_status::ready)
    {
      ...
      if (response)
      {
        ...
        for (const auto &attached_collision_object : response->scene.robot_state.attached_collision_objects)
        {
          ...
          {
            // Object found, return it
            RCLCPP_INFO(this->get_logger(), "Object '%s' data retrieved from attached objects in planning scene.",
                        object_id.c_str());
    ...
    if (future_response.wait_for(std::chrono::seconds(5)) == std::future_status::ready)
    {
      ...
      if (response)
      {
        for (const auto &attached_collision_object : response->scene.robot_state.attached_collision_objects)
        {
          ...
          {
            // Object found, return it
            RCLCPP_INFO(this->get_logger(), "Object '%s' link retrieved from attached objects in planning scene.",
                        object_id.c_str());
  ...
  ObjectManagerNode::createCollisionObject(const std::string &id, const std::string &shape,
                                           ...
                                           const std::string &mesh_file, const std::vector<double> scale_mesh) const
  {
    ...
    else
    {
      ...
      else if (shape == "cylinder")
      {
        ...
        {
          RCLCPP_WARN(this->get_logger(), "Insufficient dimensions for cylinder. Using default size.");
          primitive.dimensions[0] = 0.1;
      ...
      else if (shape == "sphere")
      {
        ...
        {
          RCLCPP_WARN(this->get_logger(), "Insufficient dimensions for sphere. Using default size.");
  ...
  // ----------------------------------------------------------------------------
  bool ObjectManagerNode::validateShapeAndDimensions(const std::string &shape, const std::vector<double> &dimensions,
                                                     const std::optional<std::string> &mesh_file) const
  {
    ...
    RCLCPP_WARN(this->get_logger(), "Invalid shape '%s' or missing required dimensions/mesh file.", shape.c_str());
